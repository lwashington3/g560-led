...
class G403(LogiBase):
	...
	Usage:
	\tg403 solid {color} - Solid color mode
	\tg403 cycle [{rate} [{brightness}]] - Cycle through all colors
	\tg403 breathe {color} [{rate} [{brightness}]] - Single color breathing
	\tg403 intro {on|off} - Enable/disable startup effect
	...
	def set_led(self, mode: LEDMode, data):
		...
		suffix = '000000000000'
		self.send_command(f"{prefix_scrollwheel}{mode}{data}{suffix}")
		self.send_command(f"{prefix_logo}{mode}{data}{suffix}")
